pipeline {
    ...
    environment {
        ...
        /* groovylint-disable-next-line LineLength */
        TARGETGROUP_UTOPIA_ORCHESTRATOR_DEV_ARN = "${sh(script:'echo $AWS_SECRET_TARGET_GROUPS | jq -r \'. | .TARGETGROUP_UTOPIA_ORCHESTRATOR_DEV_ARN \'', returnStdout: true)}"
        /* groovylint-disable-next-line LineLength */
        TARGETGROUP_UTOPIA_ORCHESTRATOR_PROD_ARN = "${sh(script:'echo $AWS_SECRET_TARGET_GROUPS | jq -r \'. | .TARGETGROUP_UTOPIA_ORCHESTRATOR_PROD_ARN \'', returnStdout: true)}"
